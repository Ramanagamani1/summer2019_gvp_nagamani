...
void search(char **a, char *str, int r, int c){
	//int count = 0;
	int t = 0, b = 1, drd = 2, dlu = 3, dld = 4, dru = 5, l = 6, r = 7;
	int k = 0;
	for (int i = 0; i < r; i++){
		for (int j = 0; j < c; j++){
			if (str[k] == a[i][j]){
				if (str[k + 1] == a[i - 1][j] && i != 0)
					 direction(a, str + 2, r, c, i - 1, j, k + 2, 0);
				if (str[k + 1] == a[i + 1][j] && i != r)
					 direction(a, str + 2, r, c, i + 1, j, k + 2, 1);
				if (str[k + 1] == a[i + 1][j + 1] && i != r&&j != c)
					 direction(a, str + 2, r, c, i + 1, j + 1, k + 2, 2);
				if (str[k + 1] == a[i - 1][j - 1] && i != 0 && j != 0)
					 direction(a, str + 2, r, c, i - 1, j - 1, k + 2, 3);
				if (str[k + 1] == a[i + 1][j - 1] && j != 0)
					 direction(a, str + 2, r, c, i + 1, j - 1, k + 2, 4);
				if (str[k + 1] == a[i - 1][j + 1] && j != c)
					direction(a, str + 2, r, c, i - 1, j + 1, k + 2, 5);
				if (str[k + 1] == a[i][j - 1] && i != 0 && j != c)
					 direction(a, str + 2, r, c, i, j - 1, k + 2, 6);
				if (str[k + 1] == a[i][j + 1] && i != r&&j != 0)
					 direction(a, str + 2, r, c, i, j + 1, k + 2, 7);
			}
		}
	}
